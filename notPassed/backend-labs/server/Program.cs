...
app.MapGet(
    "/comments/{id}",
    (int id, ApplicationContext applicationContext) =>
    {
...
app.MapPost(
    "/comments",
    (Comment comment, ApplicationContext applicationContext) =>
    {
        var createdComment = applicationContext.AddComment(comment);
        applicationContext.AddLog(
            new LogModel
            {
                Type = "Info",
                Info =
                    $"Add a new comment with data: Id = {createdComment.Id}, "
...
app.MapPatch(
    "/comments/{id}",
    (int id, Comment comment, ApplicationContext applicationContext) =>
    {
        ...
        if (updatedComment is not null)
            applicationContext.AddLog(
                new LogModel
                {
                    Type = "Info",
                    Info =
                        $"Update the comment with Id = {id}. New data: Name = {updatedComment.Name}, "
...
app.MapDelete(
    ...
    (int id, ApplicationContext applicationContext) =>
    {
        ...
        if (isDeleted)
            applicationContext.AddLog(
                new LogModel { Type = "Info", Info = $"Remove the comment with Id = {id}." }
            );
        else
            applicationContext.AddLog(
                new LogModel
